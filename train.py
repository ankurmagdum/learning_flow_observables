...
class LossWriter:
    ...
    def __call__(self, loss, opt):
        if opt == 'lbfgs':
            np.savetxt(f'{self.basename}', loss)
        else:
            np.savetxt(f'{self.basename}', loss.history['loss'])
...
""")
    ...
    if args.save_loss_output_file is not None:
        with open(args.save_loss_output_file,'a') as f:
            #f.write(str(trainer.average_loss)+'\n')
