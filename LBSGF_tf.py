...
if __name__ == "__main__":
    ...
    paras = numpy.loadtxt('paras.txt')
    drags = numpy.loadtxt('dra.txt')
    size = 256
    inps = paras[0:size]
    outs = drags[0:size]
    ...
    results = tfp.optimizer.lbfgs_minimize(
        value_and_gradients_function=func, initial_position=init_params, max_iterations=10000, f_relative_tolerance=1.0 * np.finfo(float).eps)
    ...
    # do some prediction
    pred_outs = pred_model.predict(paras[len(paras)//2:])
    err = numpy.abs(pred_outs-drags[len(paras)//2:])
    #print("L2-error norm: {}".format(numpy.linalg.norm(err)/numpy.sqrt(11)))
    print("L2-error norm: {}".format(numpy.sum(err)/(0.01622*(len(pred_outs)))))
